...
| B2 | [~20 ms/frame of unaccounted CPU on Android](bug-reports/android-cpu-time-regression.md) | perf (45-50 FPS vs expected 60) | `engine_rendering` — bgfx render thread paced by Android SurfaceFlinger | **Correction via sama `4eed082` stats diagnostic**: `bgfx::getStats().waitRender = 13.84–15.06 ms` proves the render thread DOES exist (multi-threaded engaged) — earlier verdict that bgfx was silently single-threaded was wrong. New picture: the render thread itself takes ~14 ms per frame even though its CPU+GPU work totals only ~3.7 ms; the gap is almost certainly `vkAcquireNextImageKHR` blocking on SurfaceFlinger / swapchain image-count starvation. Engine-side investigation: try requesting more swapchain images or switching to a semaphore-based acquire pattern in bgfx's Vulkan-Android backend. |
