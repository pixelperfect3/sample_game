...
| B2 | [~20 ms/frame of unaccounted CPU on Android](bug-reports/android-cpu-time-regression.md) | perf (45-50 FPS vs expected 60) | `engine_rendering` — Android SurfaceFlinger paces at `vkQueuePresentKHR` regardless of present mode | **MAILBOX patch (`089fe5b`) did NOT close gap either.** 10-sample mean `bgfx::frameMs` = 16.6 ms — still pinned to the vsync period. Variance is wide (9.21–24.77 ms) with at least one <16.7 ms sample, which is impossible under strict FIFO — so MAILBOX likely did engage, but the compositor is enforcing refresh-rate gating *at present* rather than *at acquire*. Matches the engine team's fallback hypothesis 2. Next probe: per-call timing of `vkAcquireNextImageKHR` vs `vkQueueSubmit` vs `vkQueuePresentKHR` on the render thread to confirm where the wait moved to. |
