...
| B3 | [Gyro/accel events never arrive on Android](bug-reports/android-gyro-no-events.md) | functional (no tilt input) | `engine_platform_android` — `AndroidGyro::init` | `AndroidGyro` creates its sensor event queue with `ident = ALOOPER_POLL_CALLBACK` and a null callback — an invalid combo per NDK docs; the looper silently drops the queue. `gyro.available == true` but values stay flat at `0.000` forever. One-line fix: positive ident. Tested locally — necessary but not sufficient; a second issue (likely in `ALooper_pollAll` not routing the ident in the frame loop) also exists. |
