...
| B2 | [~20 ms/frame of unaccounted CPU on Android](bug-reports/android-cpu-time-regression.md) | perf (45-50 FPS vs expected 60) | bgfx Vulkan present + display cutout — swapchain rebuilt every frame | **ROOT CAUSE FOUND** (via `BX_CONFIG_DEBUG=ON` capture): `vkQueuePresentKHR` returns `VK_SUBOPTIMAL_KHR` every frame, bgfx interprets that as "surface changed — must rebuild swapchain", calls `vkDeviceWaitIdle` + destroys + recreates the swapchain every ~15 ms. The cost of `vkDestroySwapchainKHR + vkCreateSwapchainKHR + vkDeviceWaitIdle` is what the entire B2 investigation has been measuring. Likely Pixel 9 display-cutout mismatch: activity drawable region `2251×1080`, but Vulkan surface `currentExtent` reports the full panel `2424×1080`. Two fixes: (a) treat `VK_SUBOPTIMAL_KHR` as success in bgfx's present path, or (b) create the swapchain at `currentExtent` and viewport-crop. Either drops `bgfx::frame` from ~15 ms to ~0.1 ms. |
