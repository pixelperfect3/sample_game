...
| B2 | [~20 ms/frame of unaccounted CPU on Android](bug-reports/android-cpu-time-regression.md) | perf (45-50 FPS vs expected 60) | `engine_rendering` — bgfx multi-threaded mode not winning | **Fix attempt `0824768` did not resolve**: with `EngineDesc::singleThreaded = false` opted in from our `main_android.cpp` and `BGFX_CONFIG_MULTITHREADED=1` compiled in, `bgfx::frame` is still ~15+ ms (vs engine team's predicted ~0.1 ms). Two hypotheses for engine team: (a) multi-threaded silently not engaging on Android Vulkan; (b) vsync-locked swapchain queue stall (docs' ~0.1 ms was vsync-off). Bug report updated with the post-fix data. |
