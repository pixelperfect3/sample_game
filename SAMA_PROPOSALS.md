...
| B2 | [~20 ms/frame of unaccounted CPU on Android](bug-reports/android-cpu-time-regression.md) | perf (45-50 FPS vs expected 60) | `engine_rendering` — bgfx Vulkan synchronous `vkAcquireNextImageKHR` on render thread | **`numBackBuffers=3` fix in `6a0cd65` engaged but did NOT close gap.** `dumpsys gfxinfo` confirms 3 GraphicBufferAllocator entries; frame time slightly regressed (15.6→17.2 ms, waitRender 13.8→18.7 ms). Diagnosis updated to hypothesis 3: the gate isn't swapchain depth, it's bgfx's synchronous acquire pattern on the render thread — adding images just adds in-flight latency without unblocking the serializer. Next: bgfx-upstream investigation of the `vkAcquireNextImageKHR` call site in `renderer_vk.cpp`. |
